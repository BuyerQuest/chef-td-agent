...
resource_name :fluent_filter
provides :fluent_filter
unified_mode true
...
action :create do
  ...
  template "/etc/#{node['td_agent']['conf_dir_name']}/conf.d/#{new_resource.filter_name}.conf" do
...
action :delete do
  ...
  file "/etc/#{node['td_agent']['conf_dir_name']}/conf.d/#{new_resource.filter_name}.conf" do
    action :delete
    only_if { ::File.exist?("/etc/#{node['td_agent']['conf_dir_name']}/conf.d/#{new_resource.filter_name}.conf") }
    notifies :reload, "service[#{node['td_agent']['service_name']}]"
  end
